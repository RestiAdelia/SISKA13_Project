                        ...
                        <td class="px-4 py-3 border-l border-gray-100 whitespace-nowrap text-center">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <a href="{{ route('absen.siswa.edit', ['siswa' => $s->id, 'week' => $start->toDateString()]) }}"
                                    class="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all duration-200 group"
                                    title="Lihat Detail">
                                    <button title="Edit Data" class="text-yellow-600 hover:text-yellow-800">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M11 5h2M12 4v2m7.071 1.071a5 5 0 010 7.071L9 21l-4 1 1-4 10.071-10.071a5 5 0 017.071 0z" />
                                        </svg>
                                    </button>
                                </a>
                                @endif
                                @if (Auth::user()->role !== 'orangtua')
                                    <form action="{{ route('absen.destroyWeekly', $s->id) }}" method="POST"
                                        class="delete-form">
                                        @csrf
                                        @method('DELETE')

                                        {{-- Input hidden agar controller tahu rentang tanggal yang akan dihapus --}}
