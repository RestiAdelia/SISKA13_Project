    ...
    <div class="py-10">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8" x-data="{ search: '' }">
            ...
            <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
                ...
                {{-- Toolbar --}}
                <div
                    class="p-6 border-b border-gray-100 bg-gray-50/50 flex flex-col lg:flex-row gap-4 justify-between items-center">
                    <form id="filterForm" method="GET" action="{{ route('siswa.index') }}"
                        ...
                        <div class="sm:w-40 relative">
                            ...
                            <select name="tahun_ajar" onchange="document.getElementById('filterForm').submit()"
                                ...
                                @foreach ($tahunAjarList as $ta)
                                    <option value="{{ $ta }}"
                                        {{ request('tahun_ajar') == $ta ? 'selected' : '' }}>
                                        {{ $ta }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        {{-- Filter Kelas --}}@if (auth()->user()->role !== 'orangtua')
                                ...
                                <tr class="bg-white hover:bg-gray-50 transition-colors duration-200"
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @if ($s->jenis_kelamin == 'Laki-laki')
                                            <span
                                                class="inline-flex w-8 h-8 items-center justify-center rounded-full bg-blue-100 text-blue-600 font-bold text-xs">L</span>
                                        @else
                                            <span
                                                class="inline-flex w-8 h-8 items-center justify-center rounded-full bg-pink-100 text-pink-600 font-bold text-xs">P</span>
                                        @endif
                                    </td>
